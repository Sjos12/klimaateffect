...
class AirQuality extends React.Component {
    ...
    render() {
        ...
        if (aqi >= this.state.rating.good[0] && aqi <=  this.state.rating.good[1]) {
            paragraph = 'The air quality at your location is satisfactory: There are few pollutants present in the air at your location, which pose very little to no health risk. Many countries with clean air quality, like the United States, still participate in polluting the air by building factories and transporting goods in and from other countries. Even though the air quality is quite good, there is a large chance that this will change in the future due to industrialization and modernization. See how you can help by scrolling to the bottom of this page.';
        }
        else if (aqi >= this.state.rating.medium[0] && aqi <= this.state.rating.medium[1]) {
            paragraph = 'The air quality at your location is inadequate: There are too many pollutants present in the air at your location, which pose a health risk to the general public, and can cause more serious issues to sensitive groups. The air is polluted due to greenhouse gas-emitting sources like factories or containerships. Air pollution already causes environmental and health problems, and will continue doing so if immediate action is not taken. See how you can help by scrolling to the bottom of this page.'
        }
        else if (aqi >= this.state.rating.bad[0] && aqi <= this.state.rating.bad[1]) {
            paragraph = 'The air quality at your location is severe: There are too many pollutants present in the air at your location, which can cause a serious health risk to the general public, and can cause critical issues to sensitive groups. The air is polluted due to greenhouse gas-emitting sources like factories or containerships Air pollution already creates environmental and health problems, and will continue doing so if immediate action is not taken.. See how you can help by scrolling to the bottom of this page.';
        }
        return (
